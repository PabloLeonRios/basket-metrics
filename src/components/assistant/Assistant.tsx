        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 mb-6">
          {allPlayers.map((player) => (
            <label key={player._id} className={`p-3 rounded-lg cursor-pointer text-center border-2 ${selectedPlayerIds.has(player._id) ? 'bg-blue-500 text-white border-blue-600' : 'bg-gray-100 dark:bg-gray-800 border-gray-200'}`}>
              <input type="checkbox" checked={selectedPlayerIds.has(player._id)} onChange={() => handlePlayerToggle(player._id)} className="sr-only" />
              <span className="font-medium">{player.name}</span>
            </label>
          ))}
        </div>
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <select value={situation} onChange={(e) => setSituation(e.target.value as GameSituation)} className="w-full sm:w-1/2 px-4 py-3 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
            {situations.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
          </select>
          <button onClick={handleRecommend} disabled={loading} className="w-full sm:w-auto px-8 py-3 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 disabled:bg-gray-400">
      ...
      {recommendations.length > 0 && (
        <div className="space-y-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {recommendations.map((option, index) => (
              <div key={option.title} className="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-lg">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">{option.reasoning}</p>

                <div className="space-y-3">
                  {option.lineup.map(profile => (
                    <div key={profile.playerId} onClick={() => handleProfileClick(profile)} className="group relative bg-gray-50 dark:bg-gray-800 p-3 rounded-lg cursor-pointer hover:shadow-md">
                      <p className="font-bold">{profile.name}</p>
                      <p className="text-xs text-gray-500">Idoneidad: <span className="font-semibold text-blue-500">{profile.suitabilityScore.toFixed(1)}/10</span></p>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {Array.from(profile.tags).map((tag) => <span key={tag} className="px-2 py-0.5 bg-gray-200 dark:bg-gray-700 text-xs font-semibold rounded-full">{tag}</span>)}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {option.xFactor && (
                    <div className="mt-4">
                        ...
                        <div className="bg-yellow-50 dark:bg-yellow-900/50 border border-yellow-200 dark:border-yellow-800 p-3 rounded-lg mt-1 text-sm">
                            <p className="text-gray-800 dark:text-gray-200">{option.xFactor.reasoning}</p>
                        </div>
                    </div>
                )}
              </div>
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-lg">
              <h2 className="text-2xl font-bold mb-3">Ranking General de Idoneidad</h2>
              <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                {allProfiles.map((profile) => (
                  <div key={profile.playerId} onClick={() => handleProfileClick(profile)} className={`p-3 rounded-lg flex justify-between items-center cursor-pointer ${recommendations.some(rec => rec.lineup.some(p => p.playerId === profile.playerId)) ? 'bg-green-100 dark:bg-green-900/50' : 'bg-gray-50 dark:bg-gray-800'}`}>
                    <div>
                        <p className="font-bold">{profile.name}</p>
                    </div>
                    <div className="text-right">
                        <p className="text-lg font-bold text-blue-600 dark:text-blue-400">{profile.suitabilityScore.toFixed(1)}</p>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {Array.from(modalPlayerProfile.tags).map((tag) => <span key={tag} className="px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 text-sm font-medium rounded-full">{tag}</span>)}
              </div>
            </div>
            <div>
            ...
            <button onClick={() => setIsModalOpen(false)} className="mt-6 w-full py-2 bg-gray-200 dark:bg-gray-700 rounded-lg font-semibold hover:bg-gray-300 dark:hover:bg-gray-600">Cerrar</button>
          </div>
        </div>
      )}
    </div>
