...
export default function GameTracker({ sessionId }: { sessionId: string }) {
  ...
  const [onCourtPlayerIds, setOnCourtPlayerIds] = useState<Set<string>>(
    new Set(),
  );
      ...
      try {
        ...
        if (sessionData.coach) {
          const playersRes = await fetch(
            `/api/players?coachId=${sessionData.coach}&limit=1000`,
          );
          if (playersRes.ok) {
        ...
        const allPlayersCurrent = sessionData.teams.flatMap(
          (t: TeamData) => t.players,
        );
        if (
          sessionData.sessionType === 'Partido' ||
          sessionData.sessionType === 'Partido de Temporada'
        ) {
          const onCourtStarters = new Set<string>();
          sessionData.teams.forEach((team: TeamData) => {
            team.players
              .slice(0, 5)
              .forEach((p: IPlayer) => onCourtStarters.add(p._id));
          });
          const subs = eventsData
          ...
          <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow">
            ...
            <div className="space-y-2">
              ...
              <Button
                ...
                disabled={isSessionFinished || loadingAISuggestion}
                className="w-full justify-center bg-blue-600 text-white hover:bg-blue-700 flex items-center"
              >
              ...
              <div className="space-y-1">
                {team.players.map((player) => {
                  ...
                  return (
                    <div
                      key={player._id}
                      className={`flex items-center justify-between ${!isOnCourt && isPartido ? 'opacity-50' : ''}`}
                    >
                      <button
                        onClick={() =>
                          setSelectedPlayer({
                      ...
                      >
                        ...
                        <span
                          className={`flex-grow ${isFoulDanger && selectedPlayer?.id !== player._id ? 'text-orange-600 font-semibold' : ''}`}
                        >
          ...
          <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-xs sm:text-sm">
              <Button
                ...
                className={getActionButtonClass('asistencia')}
              >
                AST
              </Button>
              <Button
                ...
                className={getActionButtonClass('robo')}
              >
                ROBO
              </Button>
              <Button
                ...
                className={getActionButtonClass('tapon')}
              >
                TAP
              </Button>
              <Button
                ...
                className={getActionButtonClass('perdida')}
              >
                PER
              </Button>
              <Button
                onClick={() => logEvent('rebote', { type: 'ofensivo' })}
                disabled={
                  !selectedPlayer ||
                  !onCourtPlayerIds.has(selectedPlayer.id) ||
                  isSessionFinished
                }
                className={getActionButtonClass('rebote_ofensivo')}
              >
                REB-O
              </Button>
              <Button
                ...
                className={getActionButtonClass('rebote_defensivo')}
              >
                REB-D
              </Button>
              <Button
                ...
                className={getActionButtonClass('falta')}
              >
                FALTA
              </Button>
              <Button
                ...
                className={getActionButtonClass('tiro_libre')}
              >
                LIBRE
              </Button>
              <Button
                ...
                className={getActionButtonClass('doble')}
              >
                DOBLE
              </Button>
              <Button
                ...
                className={getActionButtonClass('triple')}
              >
                TRIPLE
              </Button>
            </div>
